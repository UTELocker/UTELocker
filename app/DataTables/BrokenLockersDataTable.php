...
class BrokenLockersDataTable extends BaseDataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        $datatables->addColumn('action', function ($row) {
            return view('admin.brokenLockers.actions', compact('row'));
        });
        ...
        $datatables->addColumn('updated_at', function ($row) {
            return Common::formatDateBaseOnSetting($row->updated_at, user()->isSuperUser());
        });
        ...
        $datatables->addColumn('status', function ($row) {
            return view('admin.brokenLockers.status', compact('row'));
        });

        $datatables->rawColumns(['code', 'action', 'check', 'status']);

        return $datatables
            ->setRowClass(function ($row) {
                return $row->status != LockerStatus::PENDING_BROKEN ? 'disabled-row' : '';
            });
    }
    ...
    public function query(Locker $model): QueryBuilder
    {
        ...
        $query = $model
            ...
            ->where('lockers.status', LockerStatus::BROKEN)
            ->orWhere('lockers.status', LockerStatus::UNDER_MAINTENANCE)
            ->orWhere('lockers.status', LockerStatus::PENDING_BROKEN)
            ->select([
                'lockers.*',
    ...
    {
        return $this->setBuilder('broken-lockers-table', 2)
            ->parameters([
                'initComplete' => 'function () {
                   window.LaravelDataTables["broken-lockers-table"].buttons().container()
